...
def save(conn, narrator):
    ...
    """
    logger.debug(f"narrator: '{narrator}'")
    narrator_id = select_id(conn, narrator)
    if narrator_id is None:
        narrator_id = insert(conn, narrator)
    logger.debug(f"narrator_id for narrator {narrator}: {narrator_id}")
    return narrator_id


def select_id(conn, narrator):
    """
    ...
    sql_select_id = """
        ...
        WHERE
            tbl_narrator.name = ?
    """
    cur = conn.execute(sql_select_id, (narrator,))
    db_row = cur.fetchone()
