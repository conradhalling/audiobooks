...
def save(conn, translator):
    ...
    """
    logger.debug(f"translator: '{translator}'")
    translator_id = select_id(conn, translator)
    if translator_id is None:
        translator_id = insert(conn, translator)
    logger.debug(f"translator_id for translator {translator}: {translator_id}")
    return translator_id


def select_id(conn, translator):
    """
    ...
    sql_select_id = """
        ...
        WHERE
            tbl_translator.name = ?
    """
    cur = conn.execute(sql_select_id, (translator,))
    db_row = cur.fetchone()
